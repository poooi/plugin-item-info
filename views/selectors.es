...
export const iconEquipMapSelector = createSelector(
  ...
  (equipIconMap) =>
    mapValues(
      invertBy(equipIconMap, (icon) => icon),
      (arr) => map(arr, (i) => +i),
    ),
)
...
const equipAirbaseMapSelector = createSelector(
  ...
  (airbase = []) => {
    const airbaseEquipsMap = map(airbase, (squad = {}) =>
      map(squad.api_plane_info, (plane) => plane.api_slotid).filter(
        (id) => id > 0,
      ),
    )

    const equipAirbaseMap = fromPairs(
      flatMap(airbaseEquipsMap, (equips, index) =>
...
const equipsSelectorMod = createSelector(
  [equipsSelector, equipShipMapSelector, equipAirbaseMapSelector],
...
export const rowsSelector = createSelector(
  [constSelector, filterEquipsSelector],
  ({ $equips = {} } = {}, _equips) =>
    map($equips, ($equip) => {
      const equips = filter(
        _equips,
        (equip) => equip.api_slotitem_id === $equip.api_id,
      )
      const hasAlv = equips.some(
        (equip) => typeof equip.api_alv !== 'undefined',
      )
      const hasLevel = equips.some(
        (equip) =>
          typeof equip.api_level !== 'undefined' && equip.api_level > 0,
      )
      const total = equips.length
      const active = filter(equips, (equip) => equip.shipId !== 0).length
      const lvGroup = groupBy(equips, ({ api_alv, api_level }) =>
        getLevelKey(api_alv, api_level),
      )
